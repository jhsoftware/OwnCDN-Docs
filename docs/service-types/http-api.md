...
For configuration options, see [UI /  Edit Service - HTTP API](../ui/edit-service/http-api.md).
...
For details, code example in many programming languages, and to try it out directly from your browser, see  [HTTP API documentation](/docs-api/){target="_blank"}

### OpenAPI


You can get the [OwnCDN OpenAPI specification here](/docs-api/OwnCDN.openapi.json){target=_blank}.


### Authentication

A valid API key **value** (not the ID) must be specified in the `x-owncdn-key` HTTP request header.  
API keys can be configured in the UI - see [UI / API Keys](../ui/api-keys.md).
...
- versionlist - Get a list of available versions of a file ([storage service](storage.md) only).
- syncnext - Get the next replication update  ([storage service](storage.md) only).
