...
To set up replication (on the "secondary" OwnCDN instance), you first create a [Replication source](ui_replication_sources.md) with the URL of the [HTTP API](st_http_api.md) and an [API Key](ui_api_keys.md) value from the primary OwnCDN instance. Then create a new Storage service with the "Replica" setting checked, and using the above Replication source. 
...
See [UI / Edit Service - Storage](es_storage.md) for configuration options.
